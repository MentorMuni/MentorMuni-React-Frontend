...
export default function MentorLayout({ children, currentSection, onNavigate, mentor }) {
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 pl-3 border-l" style={{ borderColor: 'var(--dashboard-border)' }}>
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold">
                    {mentor?.name?.split(' ').map(n => n[0]).join('') || 'ME'}
                  </div>
                  <span className="text-sm font-semibold hidden sm:block">{mentor?.name || 'Mentor'}</span>
                </div>
              </div>
            </div>
          </div>
