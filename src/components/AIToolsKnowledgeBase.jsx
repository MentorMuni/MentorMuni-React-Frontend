...
const MM_TOOLS = [
  {
    icon: BarChart2, color: 'text-[#FF9500]', bg: 'bg-[#FF9500]/10',
    border: 'border-[#FF9500]/30', hover: 'hover:border-[#FF9500]/50',
    badge: 'START HERE', badgeCls: 'bg-[#FF9500]/15 text-[#CC7000]',
  ...
  {
    icon: MessageSquare, color: 'text-[#FF9500]', bg: 'bg-[#FFB347]/10',
    border: 'border-violet-500/25', hover: 'hover:border-violet-500/50',
    badge: 'MOST USEFUL', badgeCls: 'bg-[#FFB347]/15 text-violet-800',
  ...
  },
  {
    ...
    badge: 'HIDDEN FILTER', badgeCls: 'bg-pink-500/15 text-pink-800',
...
const typeBadge = {
  free:     'bg-green-500/15 text-green-800 border border-green-500/25',
  freemium: 'bg-amber-500/15 text-amber-900 border border-amber-500/25',
  paid:     'bg-red-500/15 text-red-800 border border-red-500/25',
};
...
function FaqItem({ q, a }) {
  ...
  return (
    <div className={`border rounded-xl overflow-hidden transition-colors ${open ? 'border-[#FF9500]/35 bg-[#FF9500]/5' : 'border-border bg-white'}`}>
      <button
        onClick={() => setOpen(v => !v)}
      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative max-w-5xl mx-auto px-6 pt-16 pb-12">
          ...
          <div className="flex flex-wrap gap-3">
            {[
              { icon: Brain,        color: 'text-[#FF9500]', label: '8 major LLMs explained' },
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-10 items-start">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { icon: Brain,     color: 'text-[#FF9500]', bg: 'bg-[#FF9500]/10', title: 'Trained on text',          desc: 'Trillions of words: books, web, code' },
                ...
                { icon: Code2,     color: 'text-[#FF9500]', bg: 'bg-[#FFB347]/10', title: 'Predicts tokens',           desc: 'Generates one word at a time probabilistically' },
                ...
                return (
                  <div key={i} className={`flex flex-col gap-2 p-4 rounded-xl border border-border ${item.bg}`}>
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-4">
            {LLMS.map((llm) => (
              <div key={llm.name} className="flex flex-col gap-3 p-4 rounded-xl border border-border bg-[#FFF8EE]/80 hover:border-[#FF9500]/30 transition-colors">
                ...
                {llm.badge && (
                  <span className="inline-block text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#FF9500]/15 text-[#CC7000] w-fit">
                    {llm.badge}
                  </span>
                )}
                <div className="flex flex-col gap-1">
                  {llm.expertise.map((e, i) => (
                    <div key={i} className="flex items-start gap-1.5">
                      ...
                      <span className="text-[11px] text-muted-foreground leading-snug">{e}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-auto pt-2 border-t border-border flex flex-col gap-1">
                  <div className="flex justify-between gap-2">
                    <span className="text-[10px] text-muted-foreground shrink-0">Best for</span>
                    <span className="text-[10px] text-muted-foreground font-medium text-right">{llm.best_for}</span>
                  </div>
                  <div className="flex justify-between gap-2">
                    <span className="text-[10px] text-muted-foreground shrink-0">Free tier</span>
                    <span className="text-[10px] text-muted-foreground font-medium text-right">{llm.free}</span>
                  </div>
                  <div className="flex justify-between gap-2">
                    <span className="text-[10px] text-muted-foreground shrink-0">Released</span>
                    <span className="text-[10px] text-muted-foreground font-medium">{llm.release}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="relative mb-6 max-w-sm">
            ...
            <input
              ...
              className="w-full pl-8 pr-4 py-2.5 text-sm bg-white border border-border rounded-lg text-foreground placeholder-[#888888] focus:outline-none focus:border-[#FF9500]/50 transition-colors"
            />
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-black text-green-800">100% Free — start here</span>
              </div>
            ...
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-black text-amber-900">Worth paying for (once employed)</span>
              </div>
              <div className="flex flex-col gap-2.5">
                {[
