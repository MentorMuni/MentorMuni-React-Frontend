...
async function composeFromRoadmap() {
  ...
  return {
    student: {
      ...
      is_individual: Boolean(session.is_individual),
      department_name: session.department_name || '',
      college_name: session.college_name || '',
      course_or_branch: session.course_or_branch || '',
      batch_year: session.batch_year ?? null,
      roll_number: session.roll_number || '',
      year: session.year ?? session.batch_year ?? null,
