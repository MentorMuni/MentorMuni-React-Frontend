...
## 11. Frontend follow-up (when BE aliases land)

When P1–P3 are on staging, FE should verify (no rewrite required if paths match):

1. Departments page stops showing “Local demo…” (`departmentsApi` `source: 'api'`).  
2. Invite HOD → email + optional `activation_token` banner.  
3. `/activate-hod` → `/auth/activate-hod` succeeds (no local fallback).  
4. HOD login with `organization_code` → dashboard; `normalizeOrgRole('DEPARTMENT_ADMIN')` → HOD.  
5. Then wire Enrollment to `/organizations/students/*` and drop local roster for students.
